        ...
        foreach ($resourceMetadata->getGraphqlAttribute($queryName, 'filters', [], true) as $filterId) {
            ...
            foreach ($this->filterLocator->get($filterId)->getDescription($resourceClass) as $key => $value) {
                ...
                if ($value['type'] instanceof GraphQLType) {
                    ...
                    $filterType = \in_array($value['type'], Type::$builtinTypes, true)
                        ? new Type($value['type'], $nullable)
                        : new Type('object', $nullable, $value['type']);
                    $graphqlFilterType = $this->convertType(
                        $filterType,
                        false,
                        $queryName,
                        $mutationName,
